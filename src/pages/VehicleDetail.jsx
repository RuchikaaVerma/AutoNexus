// pages/VehicleDetail.jsx — COMPLETE FINAL
// Keeps Person 3's exact UI style
// Adds: ChatViewer event, real shop details, feedback form, manufacturing insights
...
export default function VehicleDetail() {
  ...
  useEffect(() => {
    ...
    getVehicle(id)
      .then(v => setVehicle({ ...v, name: v.model, owner: v.owner_name,
        sensors: { brakeTemp: v.brake_temp, oilPressure: v.oil_pressure,
                   engineTemp: v.engine_temp, tirePressure: v.tire_pressure,
  ...
  if (loading) return <div className="pt-14 min-h-screen flex items-center justify-center text-white/50">Loading {id}...</div>;
  if (!vehicle) return <div className="pt-14 min-h-screen flex items-center justify-center text-red-400">Vehicle {id} not found</div>;
  ...
  return (
    <div className="pt-14 min-h-screen p-5" style={{ background: "transparent" }}>
      ...
      <button onClick={() => navigate("/")} className="flex items-center gap-2 text-white/40 text-sm mb-5 hover:text-white/70">
        ← Back to Dashboard
      </button>
      ...
      <div className="bg-gray-900 border border-white/5 rounded-xl p-5 mb-5">
        <div className="flex items-start justify-between">
          ...
          <span className="text-xs font-bold px-3 py-1.5 rounded-full uppercase"
            ...
            {vehicle.status === "critical" ? "🔴 CRITICAL" : vehicle.status === "warning" ? "🟡 WARNING" : "🟢 HEALTHY"}
          </span>
        </div>
        ...
        {eng && (
          <div className="mt-4 pt-3 border-t border-white/5 flex flex-wrap gap-3 text-xs">
            <span className={eng.sms_sent   ? "text-green-400" : "text-red-400"}>{eng.sms_sent   ? "✅ SMS Sent"   : "❌ SMS Failed"}</span>
            <span className={eng.email_sent ? "text-green-400" : "text-red-400"}>{eng.email_sent ? "✅ Email Sent" : "❌ Email Failed"}</span>
            <span className={eng.call_action === "call_now" ? "text-blue-400" : "text-yellow-400"}>
              {eng.call_action === "call_now" ? `📞 Called — ${eng.call_outcome}` : `⏰ Call at 9:00 AM`}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="bg-gray-900 border border-white/5 rounded-xl p-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xs bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 px-2 py-0.5 rounded-full">{vehicle.id}</span>
          </div>
            ...
            return (
              <div key={g.name} className="mb-4">
                ...
                <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-700" style={{ width: pct + "%", background: gc }} />
                </div>
                {g.danger && <div className="text-xs text-red-400 mt-1">⚠️ Abnormal reading</div>}
              </div>
            );
          })}
          ...
          <button onClick={() => setShowBooking(true)}
            ...
            style={{ background: "rgba(0,227,150,0.12)", border: "1px solid rgba(0,227,150,0.3)", color: "#00e396" }}>
            📅 Book Service
          </button>
        ...
        <div className="bg-gray-900 border border-white/5 rounded-xl p-4">
          ...
          {(analyzing || done) && (
            <div className="flex flex-wrap gap-1.5 mb-4 p-3 bg-gray-800 rounded-xl">
              {AGENT_CONFIG.map(a => (
                <div key={a.key} className="text-xs px-2.5 py-1 rounded-full font-bold transition-all"
                  style={{
                    background: currentAgent === a.key ? "rgba(255,170,0,0.2)" : visibleAgents.includes(a.key) ? "rgba(0,227,150,0.15)" : "rgba(255,255,255,0.05)",
                    color:      currentAgent === a.key ? "#ffaa00" : visibleAgents.includes(a.key) ? "#00e396" : "rgba(255,255,255,0.25)",
                    border:     `1px solid ${currentAgent === a.key ? "rgba(255,170,0,0.3)" : visibleAgents.includes(a.key) ? "rgba(0,227,150,0.2)" : "rgba(255,255,255,0.05)"}`,
                  }}>
                  {a.icon} {a.name}
                </div>
              ))}
          ...
          {visibleAgents.length > 0 && analysis && (
            <div className="flex flex-col gap-2 max-h-96 overflow-y-auto pr-1">
              {AGENT_CONFIG.filter(a => visibleAgents.includes(a.key)).map(a => (
                <AgentResultCard key={a.key} agentKey={a.key} config={a}
                  finding={analysis.findings?.[a.key]} vehicle={vehicle} />
              ))}
            </div>
          )}
          ...
          {done && analysis && (
            <div className="mt-3 p-3 rounded-xl text-center"
              style={{ background: vehicle.status === "critical" ? "rgba(255,69,96,0.08)" : vehicle.status === "warning" ? "rgba(255,170,0,0.08)" : "rgba(0,227,150,0.08)", border: `1px solid ${color}30` }}>
              <div className="text-sm font-bold" style={{ color }}>
                {vehicle.status === "critical" ? "🚨 URGENT ACTION REQUIRED" : vehicle.status === "warning" ? "⚠️ MAINTENANCE NEEDED" : "✅ VEHICLE HEALTHY"}
              </div>
              <div className="text-xs text-white/40 mt-1">{analysis.final_assessment?.summary}</div>
              {eng?.call_outcome === "appointment_booked" && (
                <div className="text-xs text-green-400 mt-1">✅ Appointment booked during call!</div>
              )}
            </div>
          )}
          ...
          <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 w-80">
            ...
            <div className="flex gap-3">
              <button onClick={handleBook}
                className="flex-1 py-2 bg-green-600 hover:bg-green-700 rounded-xl text-white text-sm font-bold">
                ✅ Confirm
              </button>
              <button onClick={() => setShowBooking(false)}
                className="flex-1 py-2 bg-gray-700 rounded-xl text-white text-sm">
                Cancel
              </button>
            </div>
          </div>
